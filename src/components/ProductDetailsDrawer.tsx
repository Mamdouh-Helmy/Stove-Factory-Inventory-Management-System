...
import { X, Package, Boxes, Wallet, TrendingUp, Layers, Clock, ArrowDownCircle, ArrowUpCircle } from 'lucide-react';
...
export default function ProductDetailsDrawer({ product, open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-start">
      ...
      <div className="relative bg-gray-50 w-full max-w-2xl h-full shadow-2xl flex flex-col">
        {/* Header */}
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-800">{product.name}</h2>
            <p className="text-sm text-gray-500">{product.code ? `كود: ${product.code}` : 'بدون كود'}</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-teal-50 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-teal-600 font-medium">الكمية</span>
              </div>
              <p className="text-lg font-bold text-gray-800">{formatNumber(product.current_quantity)}</p>
            </div>
            <div className="bg-blue-50 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
                <TrendingUp className="w-4 h-4 text-blue-600" />
                <span className="text-xs text-blue-600 font-medium">متوسط التكلفة</span>
              </div>
              <p className="text-lg font-bold text-gray-800">{formatCurrency(product.average_cost)}</p>
            </div>
            <div className="bg-emerald-50 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
                <Wallet className="w-4 h-4 text-emerald-600" />
                <span className="text-xs text-emerald-600 font-medium">قيمة المخزون</span>
              </div>
              <p className="text-lg font-bold text-gray-800">{formatCurrency(product.total_stock_value)}</p>
            </div>
            <div className="bg-purple-50 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
                <Package className="w-4 h-4 text-purple-600" />
                <span className="text-xs text-purple-600 font-medium">آخر سعر</span>
              </div>
              <p className="text-lg font-bold text-gray-800">{formatCurrency(product.last_unit_cost)}</p>
            </div>
          </div>
        </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {batches.length === 0 ? (
                        ...
                        <tr key={batch.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-gray-600">{formatCurrency(batch.unit_cost)}</td>
                          <td className="px-4 py-3 text-sm text-gray-600">{formatCurrency(batch.total_cost)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {transactions.length === 0 ? (
                        <tr><td colSpan={7} className="text-center py-8 text-gray-400 text-sm">لا توجد حركات</td></tr>
                      ) : transactions.map((txn) => (
                        <tr key={txn.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 text-xs text-gray-600">{formatDateTime(txn.created_at)}</td>
                          ...
                          <td className={`px-4 py-3 text-sm font-medium ${txn.quantity > 0 ? 'text-green-600' : 'text-rose-600'}`}>
                            {txn.quantity > 0 ? '+' : ''}{formatNumber(txn.quantity)}
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-600">{formatCurrency(txn.unit_cost)}</td>
                          <td className="px-4 py-3 text-sm text-gray-600">{formatCurrency(txn.total_value)}</td>
                          <td className="px-4 py-3 text-sm font-medium text-gray-800">{formatNumber(txn.balance_after)}</td>
                          <td className="px-4 py-3 text-sm text-gray-500">{txn.reason || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {tab === 'timeline' && (
                <div className="space-y-3">
                  {transactions.length === 0 ? (
                    ...
                    <div key={txn.id} className="bg-white rounded-xl border border-gray-200 p-4 flex gap-3">
